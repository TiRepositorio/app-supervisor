package apolo.supervisor.com.ventas.sd
...
class EnviarSD {
    ...
    fun enviar() {
        ...
        if (registros == 0) {
            Toast.makeText(EnviarMarcacion.context,"No existe nunguna marcacion pendiente de envio",Toast.LENGTH_SHORT).show()
        } else {
            Enviar().execute()
        }
    }
    ...
    private fun registrarCabecera(){
        val sql : String = ("SELECT COD_EMPRESA,NRO_PLANILLA,COD_VENDEDOR,"
                ...
                + "    AND NRO_PLANILLA   = '" + FuncionesUtiles.usuario["PASS"] + "' "
                + "    AND EST_ENVIO	  = 'N' "
    ...
    private fun insertarCabecera(cursor: Cursor){
        for (i in 0 until cursor.count) {
            try {
                ...
                valores.put("COD_VENDEDOR", FuncionesUtiles.usuario["PASS"])
    ...
    fun cargarCabecera(cursor: Cursor){
        ...
        for (i : Int in 0 until cursor.count) {
            ...
            cadena += "$cabecera '$codEmpresa','${FuncionesUtiles.usuario["PASS"]}','1','$codCliente"
    ...
    fun cargarDetalle(cursor: Cursor){
        ...
        for (i in 0 until cursor.count) {
            //aca
            val codEmpresa : String = FuncionesUtiles.usuario["COD_EMPRESA"].toString()
            val nroPlanilla : String = FuncionesUtiles.usuario["PASS"].toString()
            val codRepartidor = "1"
            ...
            cadena2 += "','" + FuncionesUtiles.usuario["PASS"] + "','" + FuncionesUtiles.usuario["COD_PERSONA"] + "')"
            cursor.moveToNext()
        }
    ...
    private class Enviar : AsyncTask<Void?, Void?, Void?>() {
        ...
        override fun doInBackground(vararg params: Void?): Void? {
            return try {
                respuesta = MainActivity.conexionWS.procesaAutorizaSDSup(FuncionesUtiles.usuario["PASS"].toString().trim(), cadena, cadena2)
            ...
            if (respuesta.split("*").size != 1) {    //==> Si cant de caracteres de "mensaje" no es = 1 o Si retorna mensaje
                if (respuesta.split("*")[0] == "01") {
                    ...
                    sql = ("UPDATE svm_solicitud_dev_cab set EST_ENVIO = 'S' "
                            ...
                            +  "   AND EST_ENVIO 		= 'N' ")
                    funcion.ejecutar(sql, context)
//                    funcion.mensaje(context,"Operación exitosa!", respuesta)
                    val dialogo = DialogoAutorizacion(context)
                    dialogo.dialogoAccion("ACTUALIZAR", SolicitudDevolucion.etAccion, respuesta,"Operación exitosa!","ok")
                } else {
